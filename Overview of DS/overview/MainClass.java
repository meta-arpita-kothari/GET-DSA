...
public class MainClass {
	...
	// handle the multivariable polynomail operation like getDegree or
	// displayPolyNomail
	private static void multiVariablePoly() {
		...
		while (true) {
			...
			switch (choice) {
			case 1:
				System.out.println("Enter number of variable of " + (poly.len+1)
						+ " term");
				int var = sc.nextInt();
				if(var == 0)
					break;
				...
				System.out.println("Enter coeff. of " + (poly.len+1) + " term");
				int coeff = sc.nextInt();
	
				for (int index = 0; index < var; index++) {
					System.out.println("Enter exponent of "
							+ poly.availableVar[index]);
